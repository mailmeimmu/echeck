/*
  ...
  1. Changes
    - Update booking policies to show pending bookings only to admins
...
CREATE POLICY "Engineers can view bookings"
  ...
  TO authenticated
  USING (
    -- Engineers can only see open bookings and their own bookings
    (
      EXISTS (
...
BEGIN
  ...
  IF v_booking_status != 'open' THEN
    ...
    ELSIF v_booking_status = 'pending' THEN
      -- Booking is still pending admin approval
      RAISE EXCEPTION 'هذا الطلب قيد المراجعة من الإدارة';
    ELSE
