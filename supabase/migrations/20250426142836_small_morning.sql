...
EXCEPTION
  WHEN unique_violation THEN
    RAISE EXCEPTION 'المهندس قد تم تعيينه لهذا الطلب مسبقاً';
END;
...
BEGIN
  ...
  IF v_booking_status != 'pending' THEN
    IF v_booking_status = 'open' THEN
      RAISE EXCEPTION 'الطلب قيد المراجعة من الإدارة';
    ELSIF v_booking_status = 'engineer_assigned' THEN
      RAISE EXCEPTION 'تم تعيين مهندس لهذا الطلب';
    ELSE
  ...
  -- If accepted, update booking status to open
  IF p_status = 'accepted' THEN
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'تم تحديث حالة الطلب من قبل مهندس آخر';
    END IF;
  END IF;
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    CASE OLD.status
      -- From pending, engineer can accept to make it open
      WHEN 'pending' THEN
      ...
      -- From open, admin can assign engineer
      WHEN 'open' THEN
