/*
  ...
  1. Changes
    - Update admin email constraint to use @check.sa
    - Add proper policies for admin and engineer access
    - Fix user type detection
*/
...
CHECK (email LIKE '%@check.sa' AND email NOT LIKE 'engineer@%');
...
-- Create function to check if user is engineer
CREATE OR REPLACE FUNCTION is_engineer(user_id uuid)
RETURNS boolean AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM engineers WHERE user_id = user_id AND status = 'active'
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
