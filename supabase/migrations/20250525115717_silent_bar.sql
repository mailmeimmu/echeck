/*
  # Fix inspection submission function
  
  1. Changes
    - Fix loop variable declaration in submit_inspection function
    - Use proper record type for jsonb_each iteration
...
BEGIN
  ...
  IF jsonb_typeof(p_photos) = 'object' THEN
    FOR v_photo_record IN 
      SELECT 
        key as section_key,
        jsonb_array_elements_text(value) as photo_url 
      FROM jsonb_each(p_photos)
    ...
    LOOP
      INSERT INTO inspection_photos (
        ...
        SPLIT_PART(v_photo_record.section_key, '_', 1)
      );
    END LOOP;
  END IF;
