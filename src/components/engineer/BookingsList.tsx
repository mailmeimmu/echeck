...
export const BookingsList = () => {
  ...
  useEffect(() => {
    ...
    // Prevent body scrolling when inspection form is open
  ...
  const handleAccept = async (bookingId: string) => {
    if (processingBooking || !engineerId) return;

    try {
      setError(null);
      setProcessingBooking(bookingId);

      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/accept-booking`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'حدث خطأ في قبول الطلب');
      }
